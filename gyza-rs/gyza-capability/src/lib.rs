...
mod tests {
    ...
    /// HOSTILE challenge fixture: BMP non-ASCII, an ASTRAL codepoint
    /// (surrogate pair), DEL (U+007F), and a combining mark whose NFD form
    /// differs from the NFC one above it in the same list.
    fn fixture_challenge_hostile() -> ChallengePayload {
        ChallengePayload {
            challenge_id: "chal-0001".into(),
            eval_version: "v1".into(),
            expires_at_ns: 1_700_000_000_000_000_001,
            issued_at_ns: 1_700_000_000_000_000_000,
            nonce: "n".repeat(32),
            task_ids: vec![
                "caf\u{e9}".into(),
            ...
            validator_pubkey: "v".repeat(64),
        }
    }

    const HOSTILE_CHALLENGE_EXPECTED: &str = "{\"challenge_id\":\"chal-0001\",\"eval_version\":\"v1\",\"expires_at_ns\":1700000000000000001,\"issued_at_ns\":1700000000000000000,\"nonce\":\"nnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn\",\"task_ids\":[\"caf\\u00e9\",\"x\\ud83d\\udd10y\",\"a\\u007fb\",\"cafe\\u0301\"],\"validator_pubkey\":\"vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv\"}";

    #[test]
    fn hostile_challenge_canonical_bytes_parity_with_python() {
        let got =
            String::from_utf8(challenge_canonical_bytes(&fixture_challenge_hostile()).unwrap())
                .expect("UTF-8");
        assert_eq!(got, HOSTILE_CHALLENGE_EXPECTED);
    }

    /// NEGATIVE CONTROL. The survey called this pair an INFERENCE ("same
    /// mechanism by construction") rather than a measurement. This measures
    /// it: the pre-fix encoder must FAIL here, or the fixture has no power.
    #[test]
    fn negative_control_pre_fix_encoder_fails_hostile_challenge() {
        let pre = String::from_utf8(serde_json::to_vec(&fixture_challenge_hostile()).unwrap())
            .expect("UTF-8");
        assert_ne!(
            pre, HOSTILE_CHALLENGE_EXPECTED,
            "pre-fix encoder must NOT match Python -- otherwise this fixture \
