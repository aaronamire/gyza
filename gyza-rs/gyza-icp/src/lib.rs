...
//!
//! **The divergence was not unknown -- it was ASSUMED AWAY.** This module
//! previously carried an "ASCII-only invariant" here, reasoning that no
//! envelope field contains non-ASCII in practice (UUIDs, hex, identifiers
//! like `"anthropic:claude-sonnet-4-5"`), that the two encoders therefore
//! agree, and that "if a future field needs non-ASCII, this needs explicit
//! reconciliation."
//!
//! Every clause of that was true and it was still the wrong call. **Nothing
//! enforced the invariant.** No validation restricted any field to ASCII,
//! and `gyza bundle <intent_id>` (`gyza/cli.py:2493`) takes a free-form
//! positional string, so the "future field" was reachable from the command
//! line the whole time. An unenforced invariant is an assumption, and this
//! one sat under every cross-language signature.
//!
//! The reconciliation it asked for is done: encoding goes through
//! [`gyza_canonjson`], and `fixture_payload_hostile` in the tests exercises
//! non-ASCII, astral, DEL and combining-mark content with a negative control
//! proving the pre-fix encoder fails it. Correctness no longer depends on
//! what the fields happen to contain.
//!
//! Signing discipline:
//!
...
mod tests {
    ...
    ///
    /// Carries BMP non-ASCII, a combining mark, an ASTRAL codepoint (escaped
    /// as a UTF-16 surrogate pair), DEL (U+007F -- ASCII, and escaped by
    /// Python), CJK, NBSP, an RTL mark, plus a solidus and a quote that must
    /// NOT gain an escape. Generated by regenerate_icp_fixtures.py.
    fn fixture_payload_hostile() -> EnvelopePayload {
        let mut p = fixture_payload();
        p.intent_id = "caf\u{e9}/x\u{1F510}y".into();
        p.action_id = "cafe\u{301}".into();
        p.model_identifier = "a\u{7f}b".into();
        p.inference_backend = "\u{4e2d}\u{6587}\u{a0}\u{200f}".into();
        p.input_hashes = vec!["\u{e9}".into(), "q\"q".into(), "plain".into()];
        p
    }

    /// THE PRE-FIX ENCODER, kept executable. This is what
    /// `canonical_bytes` was before gyza-canonjson: bare `serde_json`,
    /// which emits raw UTF-8.
    fn pre_fix_canonical_bytes(p: &EnvelopePayload) -> Vec<u8> {
        serde_json::to_vec(p).expect("serialize")
    }

    const HOSTILE_EXPECTED: &str = "{\"action_id\":\"cafe\\u0301\",\"agent_pubkey\":\"08ed03d0cb5efe9152a79430ddd86a97286d760bdb5955fea3688e8bb9a13ab9\",\"capability_manifest_hash\":\"cmh1cmh1cmh1cmh1cmh1cmh1cmh1cmh1cmh1cmh1cmh1cmh1cmh1cmh1cmh1cmh1\",\"duration_ms\":42,\"inference_backend\":\"\\u4e2d\\u6587\\u00a0\\u200f\",\"input_hashes\":[\"\\u00e9\",\"q\\\"q\",\"plain\"],\"intent_id\":\"caf\\u00e9/x\\ud83d\\udd10y\",\"model_identifier\":\"a\\u007fb\",\"output_hash\":\"out1out1out1out1out1out1out1out1out1out1out1out1out1out1out1out1\",\"parent_envelope_hash\":null,\"schema_version\":1,\"timestamp_ns\":1700000000000000000,\"tokens_in\":10,\"tokens_out\":20}";

    #[test]
    fn hostile_canonical_bytes_parity_with_python() {
        let got =
            String::from_utf8(canonical_bytes(&fixture_payload_hostile()).unwrap()).expect("UTF-8");
        assert_eq!(got, HOSTILE_EXPECTED);
    }

    #[test]
    fn hostile_envelope_hash_parity_with_python() {
        assert_eq!(
            envelope_hash(&fixture_payload_hostile()).unwrap(),
            "a504242db4e1682a85db9c416207402758471997460ca547750e6ae7d8d4d334"
        );
    }

    /// NEGATIVE CONTROL -- the point of this fixture.
    ///
    /// A parity fixture that cannot reproduce the divergence it was written
    /// for is not a regression test. This asserts BOTH halves:
    ///
    ///   * the PRE-FIX encoder FAILS the hostile fixture, so the fixture has
    ///     real power to detect the bug; and
    ///   * the PRE-FIX encoder PASSES the old ASCII fixture, which is exactly
    ///     why four parity fixtures agreed trivially and caught nothing.
    ///
    /// If someone reverts `canonical_bytes` to bare serde_json, the first
    /// assertion below still holds and the two tests above start failing.
    #[test]
    fn negative_control_pre_fix_encoder_fails_hostile_but_passes_ascii() {
        let hostile =
            String::from_utf8(pre_fix_canonical_bytes(&fixture_payload_hostile())).expect("UTF-8");
        assert_ne!(
            hostile, HOSTILE_EXPECTED,
            "pre-fix encoder must NOT match Python here -- if it does, this \
             fixture has no power and cannot detect the divergence"
        );

        // ...and the ASCII fixture cannot tell the two encoders apart.
        let ascii_pre =
            String::from_utf8(pre_fix_canonical_bytes(&fixture_payload())).expect("UTF-8");
        let ascii_post =
            String::from_utf8(canonical_bytes(&fixture_payload()).unwrap()).expect("UTF-8");
        assert_eq!(
            ascii_pre, ascii_post,
            "the ASCII fixture agrees under BOTH encoders -- that is why it \
